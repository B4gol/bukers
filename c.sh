...
curl -v -F "chat_id=$TELEGRAM_CHAT" -F document=@$KERNEL_DIR/AnyKernel/$zip_name.zip -F "parse_mode=html" -F caption="Build completed successfully in $((BUILD_DIFF / 60)) minute(s) and $((BUILD_DIFF % 60)) seconds
